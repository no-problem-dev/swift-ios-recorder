...
Mac 側で受信する `BonjourReceiver` の 2 本柱で構成されます。

iOS 側では `BonjourExporter` を `Session` に渡すだけで、キャプチャのたびに同一 LAN の Mac 受信機を
mDNS で自動発見して TCP で送信します。一度通ったアドレスをキャッシュするため、2 回目以降はブラウズなしで
即座に送れます（テザリング環境での安定性も向上）。
...
Mac 側では `BonjourReceiver` を起動して `AsyncThrowingStream<Record, Error>` を消費します。
`serviceName` を指定すると Bonjour で自分を広告し、iOS 側がゼロコンフィグで発見します。
...
Mac まで物理的に届くかを UI で示したい場合は `ExportReachability` を使います。
`start()` 後は `isReachable` プロパティが `@Observable` で更新されるため、SwiftUI でそのまま監視できます。
